...
int main() {
    ...
    vector<pair<int, int>> startDirections;
    ...
    for(int i: {-1, 0, 1}){
        for(int j:{-1, 0, 1}){
            ...
            if(new_direction[{input[yStart+i][xStart+j], i , j}] != make_pair(0,0)) {
                startDirections.emplace_back(i, j);
                searching.push({0, yStart, xStart, i, j});
            }
        }
    }
    visited[yStart][xStart] = true;
    input[yStart][xStart] = 'F';
    ...
    for(int x = 0; x < input[0].size(); x++){
        ...
        while(y < input.size()){
            ...
            //Pass vertical pipe
            char start = input[y][x];
            if(start == 'F' or start == '7'){
                y++;
                while(y<input.size() and input[y][x] == '|'){y++;}
                if(y >= input.size()){
                    continue;
                }
                char end = input[y][x];
                if((start == 'F' and end == 'J') or (start == '7' and end == 'L')){
                    counting = !counting;
                }
                y++;
            }else{
