...
int main() {
    ...
    searching.emplace(0,0,0,0,1,10);
    searching.emplace(0,0,0,1,0,10);
    ...
    while(!searching.empty()){
        ...
        if(xDir != 0 && steps <= 6) {
            if(valid(y+1, x, input.size(), input[0].size())) {
                if(visited.find({y+1, x, 1, 0, 9}) == visited.end() || visited[{y+1, x, 1, 0, 9}] >= val + input[y+1][x] - '0')
                    searching.emplace(val + input[y + 1][x] - '0', y + 1, x, 1, 0, 9);
            }
            if(valid(y-1, x, input.size(), input[0].size())) {
                if(visited.find({y-1, x, -1, 0, 9}) == visited.end() || visited[{y-1, x, -1, 0, 9}] >= val + input[y-1][x] - '0')
                    searching.emplace(val + input[y - 1][x] - '0', y - 1, x, -1, 0, 9);
        ...
        if(yDir != 0 && steps <= 6) {
            if(valid(y, x+1, input.size(), input[0].size())) {
                if(visited.find({y, x+1, 0, 1, 9}) == visited.end() || visited[{y, x+1, 0, 1, 9}] >= val + input[y][x+1] - '0')
                    searching.emplace(val + input[y][x+1] - '0', y, x+1, 0, 1, 9);
            }
            if(valid(y, x-1, input.size(), input[0].size())) {
                if(visited.find({y, x-1, 0, -1, 9}) == visited.end() || visited[{y, x-1, 0, -1, 9}] >= val + input[y][x-1] - '0')
                    searching.emplace(val + input[y][x-1] - '0', y, x-1, 0, -1, 9);
